...
class AdminCategoriesController extends Controller
{
    ...
    public function fileoptimize(Request $request, $type)
    {
        ...
        foreach ($categories as $category) {
            ...
            foreach ($files as $key => $file) {
                try {
                    Image::make($file)->resize(1200, null,
                        function ($constraint) { $constraint->aspectRatio(); })->save($file, 90);
    ...
    public function edit($type = null)
    {
        ...
        return view('backend.categories.edit')->with(compact('langs', 'admin_category', 'type', 'attributes_fields',
            'category_parent', 'article_parent'))->with(['action_method' => 'put']);
    ...
    public function update(Request $request, $type)
    {
        ...
        foreach ($files as $key => $file) {
            ...
            try {
                $image = Image::make($file)->resize(1200, null,
                    function ($constraint) { $constraint->aspectRatio(); })->save($savePathFull, 80)->resize(320, null,
                    function ($constraint) { $constraint->aspectRatio(); })->save($savePathMin, 80);
